...
> 2026-09-12 清理：上面这些痕迹已全部删除 —— `~\.codex\switcher-backups\` 与 `~\.codex\backup-deepseek\` 里的 `config.toml` 是明文 API key 的副本，`~\.codex\desktop-deepseek-switch.json` 是切换器的状态文件（记录的是早已不存在的 `~\.codex\models.json` 路径），`~\.codex\rules\default.rules` 里那条指向旧目录的 `set-path` 放行规则也一并移除了。旧项目目录 `D:\code\multica-codex-deepseek` 被本仓库取代，同样已删；Multica 用的是 `~\.multica\bin` 里的独立副本，不受影响。
